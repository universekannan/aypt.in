...
if (isset($_POST['submit'])) {
         ...
         $stmt->bind_param("ss", $pro_old_balance=$balance, $id=$p_id);
         $stmt->execute();
		
		 $stmt1 = $conn->prepare("INSERT INTO galaxy_amound(date,description,payed,balance,old_balance,user_id,project_id) VALUES (?,?,?,?,?,?,?)");
        $stmt1->bind_param("sssssss",$date,$description,$payed,$balance,$old_balance=$pro_old_balance+$payed,$user_id,$project_id=$p_id);
        $stmt1->execute()or die($stmt1->error);
        $id=$stmt1->insert_id;

        header("location: amound.php?id=$p_id");
    }


$sql = "select * from galaxy_project where id=$p_id";
